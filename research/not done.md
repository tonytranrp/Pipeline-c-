...
| Sequential fan-in join surface | First production slice implemented. | Public `::fan_in<JoinStage>` and `::join_all<JoinStage>` follow `::branch<...>`, evaluate all predicates, run every passing case in declaration order, and pass `pb::fan_in_results<pb::fan_in_case_result<I, T>...>` to the join stage. Targeted compile-pass, runtime, and compile-fail tests cover zero/one/many passing cases, duplicate same-type case outputs by index, stateful storage, invalid fan-in joins, non-copyable fan-in inputs, and void-output fan-in case rejection. |
...
| --- | --- | --- |
| Branch/join execution | First-match selected-output sequential branch execution, optional join stages, homogeneous outputs, variant heterogeneous outputs, type-list selected-output joins, explicit sequential fan-in joins, stateful branch storage, observer events, and move-only selected-stage consumption. | Backend/parallel branch execution; richer branch/fan-in result aggregation; fan-in failed-case aggregation instead of first error return; void-output fan-in aggregation; backend ordering/cancellation rules; full backend branch tests. |
| Heterogeneous branch outputs | `std::variant` preserves duplicate alternatives by index; type-list selected-output joins dispatch by C++ type and intentionally share overloads for duplicate same-type outputs unless the user encodes case identity into the output type. Sequential fan-in distinguishes duplicate same-type case outputs by case index in `fan_in_case_result<I, T>`. | Tagged case-result model for selected-output joins if needed; richer fan-in failed-case policies; docs/examples for all future policies. |
| Move-only branch input | Predicates observe by `const input_type&`; selected branch stage may consume move-only input. Compile-fail coverage rejects predicates that consume move-only inputs by value. Fan-in currently requires copy-constructible input because one input may feed multiple passing cases; this boundary has targeted compile-fail coverage. | Broader clone/borrow/shared-view policies; resource lifetime docs; more observer/error edge cases; fan-in policy for inputs that cannot be copied to multiple branches. |
...
The repository now has an explicit sequential fan-in slice: `::fan_in<JoinStage>` and `::join_all<JoinStage>` evaluate every predicate, run every passing case in declaration order, allow zero passing cases, and invoke the join with an ordered `pb::fan_in_results<...>` aggregate. This is a real implemented feature, but it is not the full research-plan graph/backend fan-in story yet.

Still need to implement:

```text
- richer failed-case aggregation instead of returning on the first predicate/stage error
- cancellation policy once parallel/backend fan-in exists
- copy/clone/borrow policy for fan-in inputs that are not copy-constructible
- observer events beyond selected/skipped, such as case scheduled/completed/failed and join started/completed
...
8. Harden the new sequential fan-in slice with richer failure aggregation, fan-in descriptor/export docs, and non-copyable input policy design.
...
Sequential fan-in join execution:          6.8 / 10
...
It is still not the full research-plan product. The remaining maximum-production work is concentrated in hardening the new sequential fan-in slice into backend/parallel fan-in with richer aggregation policies, stable descriptor/export contracts, optional pipeline backends, full policy/lifetime/error semantics, release-grade performance budgets, C++ modules/C++26 gates, and final exact-SHA release publication.
