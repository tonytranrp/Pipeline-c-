...
- ctest --preset clang-dev-ninja --output-on-failure: passed, 240/240
...
| Release evidence | Exact-SHA local, CI, cross-compiler, and package evidence exists for code SHA `87299c14c813753d170911239e251064cbbfee6f`. Working tree is **240/240 local ctest**; cross-compiler validation on the new SHA is pending. | `docs/cross-compiler-validation.md`, `docs/current-release-summary.md`, `docs/production-readiness.md`, `docs/research-verification-matrix.md` |
...
```text
- decide final release candidate SHA (current integrated team head, 240/240 ctest)
...
```text
1. Rerun cross-compiler validation on the integrated team head (240/240 ctest, all wave-1/2/3 plus post-wave API/docs features).
...
Release readiness before tag:             7.5 / 10  (was 8.2 — new SHA not yet cross-compiler-validated; ctest 240/240)
