...
- Compile-pass, compile-fail, runtime, example, package, compile-time/header benchmark smoke scaffolding, and cross-compiler validation workflow. Targeted thread-pool backend fan-in runtime coverage is present. Current working tree: **200/200 local ctest** (post-release-readiness pack — schema v1 cross-version regression, observer ABI + verbose line schema regression, error JSON serialization, NDJSON tracing sink, expanded compile-fail diagnostic goldens, compile-time benchmark baselines); cross-compiler validation on the new SHA pending.
...
Current working tree state: **200/200 local ctest** (includes the error-policy DSL parity-hardening batch plus the release-readiness pack: schema v1 stability promise + cross-version regression, observer ABI `pb.observer.v1` + verbose line-schema `pb.observer.verbose.v1` regression, `pb.error.v1` JSON serialization for `error_record`, `pb.trace.ndjson.v1` streaming `pb::tracing_sink` observer, expanded compile-fail diagnostic goldens, recorded compile-time benchmark baselines). Cross-compiler validation on the new working-tree SHA has not yet been rerun. Rerun the cross-compiler workflow on the final candidate SHA before any release-facing claim.
