...
- Error-policy DSL: five factory-built, engine-side wrappers — `pb::with_throw_on_error` → `throwing_engine` (throws `pb::pipeline_exception` on failure), `pb::with_terminate_on_error` → `terminating_engine` (calls `std::terminate()` on failure), `pb::with_ignore_errors` → `ignoring_engine` (returns user-supplied fallback on failure), `pb::with_propagate_exceptions` → `propagating_engine` (re-throws exceptions captured as `error_category::exception`), `pb::with_verbose_diagnostics` → `verbose_engine` (owns a `pb::verbose_observer` that logs every transition to a `std::ostream` under the `[pb.verbose]` prefix). Header: `include/pb/runtime/error_policy.hpp`. Tests: `tests/runtime/throw_on_error_smoke.cpp`, `tests/runtime/policy_dsl_engine_smoke.cpp`. Cross-wrapper `run()` / `try_run()` parity is now proven over linear, selected-output branch, and explicit fan-in topologies — for each (wrapper, method, case) combination across success, declared `result<>` failure, and thrown exception — via the parity matrix tests (`tests/runtime/policy_dsl_parity_matrix_linear.cpp`, `tests/runtime/policy_dsl_parity_matrix_branch.cpp`, `tests/runtime/policy_dsl_parity_matrix_fan_in.cpp`). The wrappers also compose: `verbose_engine` can wrap any of the other four wrappers (`tests/runtime/policy_dsl_composability_smoke.cpp`), because all five expose `set_observer` / `get_observer` / `describe` / `descriptor` directly. `std::expected`-returning stages flow through every wrapper with `error_category::expected_error` semantics where `PB_HAS_STD_EXPECTED == 1` (`tests/runtime/policy_dsl_std_expected_matrix.cpp`).
...
- Compile-pass, compile-fail, runtime, example, package, compile-time/header benchmark smoke scaffolding, and cross-compiler validation workflow. Targeted thread-pool backend fan-in runtime coverage is present. Current working tree: **201/201 local ctest** (post-release-readiness pack + stateful storage DSL — schema v1 cross-version regression, observer ABI + verbose line schema regression, error JSON serialization, NDJSON tracing sink, expanded compile-fail diagnostic goldens, compile-time benchmark baselines, `pb.state.v1` thread-local state DSL with four storage policies); cross-compiler validation on the new SHA pending.
- Stateful storage DSL (`pb.state.v1`): thread-local `pb::current_state<S>()` / `pb::try_current_state<S>()` plus four engine-wrapper factories — `pb::with_state<S>(engine)` (owned, persists across runs), `pb::with_borrowed_state<S>(engine)` (caller supplies state per call via `run_with_state`), `pb::with_shared_state<S>(engine, ptr)` (`shared_ptr<S>` across engines/threads), `pb::with_reset_per_run_state<S>(engine)` (owned + reset before each run). Composes with every error-policy wrapper (`set_observer` / `get_observer` / `describe` / `descriptor` forwarding parity); exception-safe via `pb::state_context<S>` RAII frame; multi-state works via nested wrappers since each state type has an independent thread-local stack. Header: `include/pb/runtime/state.hpp`. Docs: `docs/state-dsl.md`. Test: `tests/runtime/state_dsl_smoke.cpp`. Schema-pinned in `tests/compile_pass/schema_v1_cross_version_regression.cpp`.
...
Current working tree state: **201/201 local ctest** (includes the error-policy DSL parity-hardening batch, the release-readiness pack — schema v1 stability promise + cross-version regression, observer ABI `pb.observer.v1` + verbose line-schema `pb.observer.verbose.v1` regression, `pb.error.v1` JSON serialization for `error_record`, `pb.trace.ndjson.v1` streaming `pb::tracing_sink` observer, expanded compile-fail diagnostic goldens, recorded compile-time benchmark baselines — and the stateful storage DSL `pb.state.v1` with thread-local `current_state<S>()` plus four storage policies and full composability with the error-policy wrappers). Cross-compiler validation on the new working-tree SHA has not yet been rerun. Rerun the cross-compiler workflow on the final candidate SHA before any release-facing claim.
...
| Error-policy DSL | Five named wrappers shipped: `throwing_engine`, `terminating_engine`, `ignoring_engine`, `propagating_engine`, `verbose_engine` (with owned `verbose_observer`). `pb::pipeline_exception` carries the runtime diagnostic. Cross-wrapper `run()` / `try_run()` parity is proven across linear, branch, and fan-in topologies via the parity matrix tests. Composability is shipped: `verbose_engine` can wrap any of the other four wrappers because all five expose `set_observer` / `get_observer` / `describe` / `descriptor` directly. `std::expected`-shape stages flow through every wrapper with `error_category::expected_error` normalization. | Runtime-enforced `::with<pb::policy::...>` DSL beyond the marker-bundle introspection seam, and unified policy-surface narrative docs, remain roadmap-only. |
...
- The error-policy DSL is shipped and parity-hardened: five factory-built wrappers (`throwing_engine`, `terminating_engine`, `ignoring_engine`, `propagating_engine`, `verbose_engine`) are tested in `tests/runtime/throw_on_error_smoke.cpp`, `tests/runtime/policy_dsl_engine_smoke.cpp`, plus the new parity matrix lane (`tests/runtime/policy_dsl_parity_matrix_{linear,branch,fan_in}.cpp`), composability lane (`tests/runtime/policy_dsl_composability_smoke.cpp`), and `std::expected` lane (`tests/runtime/policy_dsl_std_expected_matrix.cpp`). The remaining gap is a runtime-enforced `::with<pb::policy::...>` DSL over every policy axis, not the wrapper layer.
