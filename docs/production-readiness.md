...
Current local fan-in candidate snapshot: this fan-in commit passed the developer and package-release verification loop on 2026-05-19: `git diff --check`, fan-in hardening checks, `clang-dev-ninja` configure/build/full CTest `162/162`, GCC/default `dev-ninja` configure/build/full CTest `162/162`, targeted fan-in CTest `6/6`, `package-release-clang-ninja` configure/build/CTest `162/162`, and TGZ package generation (`build/package-release-clang-ninja/pipebuilder-0.1.0-Linux.tar.gz`). Rerun exact-SHA GitHub validation before release tagging.

Latest GitHub cross-compiler validation snapshot: code SHA `f56fa54399a7a6a4f1dd55433634f13aee9c3174` passed the cross-compiler workflow on 2026-05-19: GCC C++20/C++23 `153/153`, Clang C++20/C++23 `153/153`, MSVC C++20 `152/152`, and clean Ubuntu `package-release-clang-ninja` `153/153` plus TGZ package generation. See [Cross-Compiler Validation Status](cross-compiler-validation.md). A final release tag should rerun this workflow because the current fan-in commit contains later non-doc helper-export, compile-time benchmark, and sequential fan-in hardening.
...
- Local verification passed for this fan-in candidate with `clang-dev-ninja`, GCC/default `dev-ninja`, and `package-release-clang-ninja` (`162/162` each plus package generation for the package preset). Cross-compiler validation has GitHub evidence for GCC C++20/C++23, Clang C++20/C++23, MSVC C++20, and clean Ubuntu package-release on older code SHA `f56fa54`; rerun on the final tag candidate because later non-doc changes landed. MSVC C++23, Windows package-release, C++26 gates, and other platform/package-manager matrices remain unclaimed.
