...
- Sequential branch execution with optional join stages, observer case events, stateful branch storage, branch-child fallback identities, heterogeneous branch outputs represented as `std::variant`, and move-only selected-branch input consumption when predicates observe by `const input_type&`.
...
Post-`aa3b8f6` branch hardening promoted sequential branch execution into the supported surface. Current branch/join evidence includes public `::branch<...>` and `::join<...>` DSL support, homogeneous branch-output validation, first-slice heterogeneous outputs via `std::variant`, join validation, first-match-wins sequential routing, move-only selected-branch input consumption, observer case events, stateful branch predicate/stage storage, branch runtime tests, branch examples, and targeted branch export compile-pass checks. Type-list/multi-input joins, descriptor-backed stable graph export, and backend branch execution remain roadmap-only until their own implementation and verification slices land.
...
- DOT/JSON export helpers now cover linear and supported branch pipelines, including branch topology in JSON. Full graph export remains roadmap-only until descriptor-backed schema decisions, golden outputs, CLI/file behavior, examples, and compatibility tests land.
