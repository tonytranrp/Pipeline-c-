...
The package-consumer smoke script is not green at the current tree: it generates `pipebuilder_core_target_smoke`, `pipebuilder_runtime_target_smoke`, and `pipebuilder_pipeline_alias_smoke`, then still looks for the older `pipebuilder_package_smoke` executable name. Treat `pb_package_config_smoke` as a blocking package-verification bug until the CMake/package lane fixes the executable lookup and a fresh package preset run passes.
...
The package release preset in [Build and Verification](build.md) is the release-readiness gate for this contract. The `pb_package_config_smoke` test is meant to install the current build into a temporary prefix, verify that `find_package(pipebuilder CONFIG REQUIRED)` defines `pb::core`, `pb::runtime`, and `pb::pipeline`, then build separate consumers against each imported target. Treat that test as the minimum package-consumer contract only after the current executable-name blocker is fixed and the test passes on the release candidate SHA.
...
- The debug package preset and release Clang package preset both configure, build, and pass CTest after the package-smoke executable lookup is fixed.
- `pb_package_config_smoke` passes in the release package preset; until then, the package-consumer gate remains blocked.
...
- The package-consumer smoke executable lookup is currently inconsistent with the target-specific consumer executables generated by `cmake/PBPackageSmoke.cmake.in`.

## Small production-readiness slice (safe, immediate)

For the next minimal safe release-hardening step, first fix the package-smoke executable lookup, then execute and archive:
