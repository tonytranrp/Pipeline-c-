...
- Compile-pass, compile-fail, runtime, example, package, compile-time/header benchmark smoke scaffolding, and cross-compiler validation workflow. Targeted thread-pool backend fan-in runtime coverage is present; full cross-compiler/package evidence must be refreshed after this non-doc code slice.

## Current release gate status

Current validated code snapshot: code SHA `8ae7d596facc59873dc75a31d5c23cdc8cf06763` (`Harden fan-in so joins can inspect every case`) passed the local developer and package-release verification loop on 2026-05-19: `git diff --check`, `clang-dev-ninja` build/full CTest `162/162`, GCC/default `dev-ninja` build/full CTest `162/162`, `package-release-clang-ninja` build/CTest `162/162`, and TGZ package generation (`build/package-release-clang-ninja/pipebuilder-0.1.0-Linux.tar.gz`).

Latest GitHub cross-compiler validation snapshot: code SHA `8ae7d596facc59873dc75a31d5c23cdc8cf06763` passed the cross-compiler workflow on 2026-05-19: GCC C++20/C++23 `162/162`, Clang C++20/C++23 `162/162`, MSVC C++20 `161/161`, and clean Ubuntu `package-release-clang-ninja` `162/162` plus TGZ package generation. The matching normal CI workflow also passed the Clang dev, package-release, and benchmark-smoke lanes. See [Cross-Compiler Validation Status](cross-compiler-validation.md). Rerun the workflow if later non-doc code changes land before release tagging.

Working-tree validation for the backend fan-in/export/policy slice passed locally after the latest GitHub snapshot: `git diff --check`, `cmake --preset clang-dev-ninja`, `cmake --build --preset clang-dev-ninja`, `ctest --preset clang-dev-ninja --output-on-failure` (`163/163`), `cmake --preset package-release-clang-ninja`, `cmake --build --preset package-release-clang-ninja`, `ctest --preset package-release-clang-ninja --output-on-failure` (`163/163`), and `cmake --build --preset package-release-clang-ninja --target package`. Rerun GitHub cross-compiler validation on the final commit SHA before tagging because this slice changes runtime/backend code.
...
- Local verification passed for the backend fan-in/export/policy working tree with `clang-dev-ninja` and `package-release-clang-ninja` (`163/163` each plus package generation for the package preset). Earlier GitHub matrix evidence remains tied to code SHA `8ae7d596facc59873dc75a31d5c23cdc8cf06763`. GitHub cross-compiler validation for the same SHA passed GCC C++20/C++23, Clang C++20/C++23, MSVC C++20, and clean Ubuntu package-release. MSVC C++23, Windows package-release, C++26 gates, and other platform/package-manager matrices remain unclaimed.
