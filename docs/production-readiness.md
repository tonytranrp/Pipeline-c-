...
- Branch, join, graph export, observer hooks, and optional backend execution are roadmap items, not current guarantees. See [Graph Export Roadmap / Status](graph-export-roadmap.md) and [Observer Hooks Roadmap / Status](observer-hooks-roadmap.md) for the current roadmap-only boundaries and planned follow-on work.
