...
- Homogeneous sequential branch execution with optional join stages, observer case events, stateful branch storage, and branch-child fallback identities for unnamed predicates/stages.
...
Post-`aa3b8f6` branch hardening promoted homogeneous sequential branch execution into the supported surface. Current branch/join evidence includes public `::branch<...>` and `::join<...>` DSL support, homogeneous branch-output validation, join validation, first-match-wins sequential routing, observer case events, stateful branch predicate/stage storage, branch runtime tests, branch examples, and a targeted compile-fail test for the move-only branch-input limitation. Heterogeneous branch/join execution, move-only branch input execution, full graph export, JSON export, and backend branch execution remain roadmap-only until their own implementation and verification slices land.
...
- Branch/join marker aliases plus `branch_case_output` / `branch_outputs` marker metadata and unsupported-boundary, branch source/predicate, homogeneous branch-node case-input, branch output compatibility validation, join consumption validation, invalid join-stage, and branch-output marker misuse diagnostics now exist. Sequential branch execution with homogeneous branch outputs, join stages, observer events, and stateful storage is now supported with runtime tests, examples, and compile-time join validation. Heterogeneous branch/join with type-list/variant join inputs and move-only branch inputs remain roadmap-only. A stable runtime descriptor/export contract also remains roadmap, not a current guarantee. See [Research Verification Matrix](research-verification-matrix.md), [Branch / Join Roadmap / Status](branch-join-roadmap.md), [Graph Export Roadmap / Status](graph-export-roadmap.md), [Observer Hooks Roadmap / Status](observer-hooks-roadmap.md), [Optional Backends Roadmap / Status](optional-backends-roadmap.md), and [Runtime Descriptor Roadmap / Status](runtime-descriptor-roadmap.md) for current boundaries.
