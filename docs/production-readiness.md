...
Current local candidate snapshot: HEAD `4d7127c947213c30f994ad39a06b8f2119687596` passed the developer and package-release verification loop on 2026-05-18: `clang-dev-ninja` configure/build/full CTest `156/156`, targeted branch/export/descriptor/benchmark CTest `59/59`, `pb_compile_time_benchmarks`, `package-release-clang-ninja` configure/build/CTest `156/156`, and TGZ package generation (`build/package-release-clang-ninja/pipebuilder-0.1.0-Linux.tar.gz`).

Latest GitHub cross-compiler validation snapshot: code SHA `f56fa54399a7a6a4f1dd55433634f13aee9c3174` passed the cross-compiler workflow on 2026-05-19: GCC C++20/C++23 `153/153`, Clang C++20/C++23 `153/153`, MSVC C++20 `152/152`, and clean Ubuntu `package-release-clang-ninja` `153/153` plus TGZ package generation. See [Cross-Compiler Validation Status](cross-compiler-validation.md). A final release tag should rerun this workflow because current HEAD contains later non-doc helper-export and compile-time benchmark hardening.
...
Post-`aa3b8f6` branch hardening promoted sequential branch execution into the supported surface. Current branch/join evidence includes public `::branch<...>` and `::join<...>` DSL support, homogeneous branch-output validation, explicit unified-output validation for variant execution output, first-slice heterogeneous outputs via `std::variant`, unified-output join validation plus selected-output type-list join dispatch with raw output type-list metadata retained, first-match-wins sequential routing, const-input predicate evaluation in per-run and stateful execution, move-only selected-branch input consumption plus bad-predicate negative coverage, observer case events, stateful branch predicate/stage storage, branch runtime tests, branch examples, and targeted branch export compile-pass checks. Parallel fan-in joins, stable descriptor/export compatibility, and backend branch execution remain roadmap-only until their own implementation and verification slices land.
