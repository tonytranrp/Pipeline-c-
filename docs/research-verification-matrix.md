...
| Branch/join topology | Public `::branch<CaseA, CaseB>::to<Output>` and `::branch<CaseA, CaseB>::join<JoinStage>::to<Output>` DSL with compile-time validation, runtime sequential branch execution (first-match-wins, short-circuit), observer events (`on_case_selected`, `on_case_skipped`), stateful storage, join stages, error propagation, and comprehensive tests and examples. | Compile-pass tests including `branch_join_validation_compile_pass.cpp`, compile-fail diagnostics for join-without-branch and join-type-mismatch, runtime branch tests (3 files), branch examples (2 files), `docs/branch-join-roadmap.md`. | Supported for homogeneous branch outputs with optional join stages. Heterogeneous branch/join with type-list/variant join inputs and move-only inputs remain roadmap-only. | Keep heterogeneous branch/join and move-only input support as separate design/implementation phases. |
...
| Optional backends | No thread-pool, oneTBB, Taskflow, or stdexec executor is supported today. A public backend feature matrix now marks only `sequential` as supported/default-build, and CMake fails fast if reserved external backend flags are enabled. | `include/pb/runtime/backend.hpp`, `pb_runtime_backend_feature_matrix_smoke`, fail-fast configure checks for `PB_BACKEND_TASKFLOW`, `PB_BACKEND_TBB`, and `PB_BACKEND_STDEXEC`, and `docs/optional-backends-roadmap.md`. | Matrix/gating done; non-sequential execution remains roadmap-only. | Prototype one backend without leaking backend APIs into core headers, then add adapter tests, examples, dependency isolation evidence, and benchmark data before any support claim. |
...
| 11-12 | Phase 7 compiler/release requirements | Release compiler matrix is documented; release/package evidence still must be collected on the candidate SHA. | Fresh compiler ID/version plus configure/build/test/package logs before tagging. |
