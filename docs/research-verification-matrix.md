...
| Branch/join topology | No supported branch or join graph surface is documented as shipped. | Roadmap-only docs: `docs/branch-join-roadmap.md` and `docs/roadmap-gap-map.md`. | Roadmap-only. | Worker 1 research/implementation slice must define branch type model, predicate requirements, branch output validation, join validation, sequential execution, and diagnostics before release claims change. |
