...
| Branch/join topology | Marker-only `case_` / `branch_case` / `branch_node` aliases and predicate marker validation now lock the unsupported boundary from research lines 468-479, 583-591, 989-997, and 1451-1462. No branch output validation, join validation, or sequential branch execution is shipped. | Compile-pass public-header marker checks and compile-fail unsupported-boundary/predicate diagnostics; `docs/branch-join-roadmap.md`. | Roadmap-only for executable branch/join topology; marker diagnostics only. | Implement branch output validation, join validation, sequential branch execution, examples, and release-facing docs before support claims change. |
