...
| Branch/join topology | Marker-only `case_` / `branch_case` / `branch_node` / `join_node` aliases plus branch source compatibility, predicate marker validation, and invalid join-stage diagnostics now lock the unsupported boundary from research lines 468-479, 583-591, 989-997, and 1451-1462. No branch output modeling, join compatibility validation, or sequential branch execution is shipped. | Compile-pass public-header marker checks and compile-fail unsupported-boundary, branch source/predicate, and invalid join-stage diagnostics; `docs/branch-join-roadmap.md`. | Roadmap-only for executable branch/join topology; marker diagnostics only. | Implement branch output modeling, join compatibility validation, sequential branch execution, examples, and release-facing docs before support claims change. |
...
| Stable runtime descriptor | Current describe/metadata helpers support linear introspection, descriptor alias symmetry coverage, and a runtime `error_record` / `to_record(...)` diagnostic projection, but no stable runtime descriptor/export contract is promised. | `include/pb/core/describe.hpp`, `include/pb/runtime/error.hpp`, descriptor compile-pass tests, runtime diagnostic smoke tests, `docs/runtime-descriptor-roadmap.md`. | Partial MVP introspection only; stable descriptor remains roadmap. | Define ownership, value schema, versioning, and export compatibility before presenting descriptor output as a stable public contract. |
