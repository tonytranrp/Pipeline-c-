...
| --- | --- | --- | --- |
| 1 | `run()` / `try_run()` error-policy hardening and exception behavior evidence | **DSL shipped; parity hardening in progress.** Five named wrappers are factory-built and engine-tested: `pb::with_throw_on_error` → `throwing_engine`, `pb::with_terminate_on_error` → `terminating_engine`, `pb::with_ignore_errors` → `ignoring_engine`, `pb::with_propagate_exceptions` → `propagating_engine`, `pb::with_verbose_diagnostics` → `verbose_engine` (owns a `pb::verbose_observer`). Smoke coverage: `tests/runtime/throw_on_error_smoke.cpp`, `tests/runtime/policy_dsl_engine_smoke.cpp`. `run()` / `try_run()` policy parity across all wrappers and all topology combinations is still being hardened. | Full policy-surface docs plus parity tests proving consistent behavior across all five wrappers on every supported topology. |
...
- current working tree at **185/185 local ctest**; cross-compiler validation on the new SHA pending
...
- exception-policy parity between `run()` and `try_run()` as a fully harmonized runtime guarantee (DSL is shipped; cross-wrapper parity is in progress)
