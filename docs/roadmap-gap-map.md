...
| 14 | Release/package evidence on candidate SHA | Current working tree at 243/243 ctest. Last cross-compiler-validated SHA has local full/package validation plus GitHub CI evidence. | Rerun package-release on the integrated team head and record archive path plus workflow URL. |
...
- current working tree at **243/243 local ctest** on `clang-dev-ninja`; sender/receiver module export syntax probe passes; `modules-ninja` was not refreshed locally because `clang-scan-deps` is missing; cross-compiler validation on the new SHA pending
